...
void clear_b(
	...
	for(e=0;e<366;e++){
		...
		for(f=0;f<DSTEP;f++){
			loct->solhgt_h[e][f] = 0.0;
			loct->toprad_h[e][f] = 0.0;
		}
		tmp_sfc_dav[e] = 0.0;
		tmp_2m_dav[e] = 0.0;
	...
	(echar->tree).grw_pd = (echar->c3).grw_pd = (echar->c4).grw_pd = 0.0;
	(echar->tree).gdd = (echar->c3).gdd = (echar->c4).gdd = 0.0;
	(echar->tree).season = (echar->c3).season = (echar->c4).season = 0;
	...
	/* added for revied phenology scheme: 2012/01/06 (A.Ito) */
	(echar->tree).ncd = (echar->c3).ncd = (echar->c4).ncd = 0.0;
	(echar->tree).gddth = (echar->c3).gddth = (echar->c4).gddth = 0.0;
	(echar->tree).gddjan = (echar->c3).gddjan = (echar->c4).gddjan = 0.0;
