...
void f_n2o_emit_ngas(
	...
	double nh4_soil;		/* micro g g-1*/ /* assumed */
	double no3_soil;						/* assumed */
...
/*	if(WSITE == TKY){
	...
	/* nitrification */
	(flux->soil).n_nitrif = day_n_n2o * 100.0;
}
...
void f_n2o_emit_ngas_2(
	...
	double aa, dd, ee, ff, gg, hh, ii;
	double n_ph, day_n_n2o;
	double fd_wfps, fd_no3, fd_co2, dt, l_m, x_i, co2_soil;
	double fr_wfps, fr_no3_co2, k1;
	double day_d_n2, day_d_n2o;
	double co2_emit;
	double nh4_soil;		/* micro g g-1*/ /* assumed */
	double no3_soil;						/* assumed */
	...
	double fno3;			/* soil nitrification rate, g N m-2 d-1 */
	double n_k1 = 0.2;		/* fraction of d_n_min from Parton et al. (2001) */
	double f_nh4;			/* model-derived soil ammonium conc., g N m-2 */
	double kmax = 0.1;		/* maxmum fraction of nh4; nitrified, d-1 from Parton et al. (2001) */
	double f_t;			/* effect of soil temperature on nitrification form Fig.(3a) in Parton et al. (2001) */
	double f_wfps;			/* the effect of SWC and soil texture on nitrificcation form Fig.(3b) in Parton et al. (2001) */
	double fn2o;			/* N2O flux from nitrification, g N m-2 d-1*/
	double n_k2 = 0.02;		/* fraction of nitrified N lost as N2O flux */
	double r_nox;			/* ratio of NOx to N2O fluxes */
	double nox;			/* potential soil gas glux of NOx, g N ha-1 d-1 */
	double st_min, st_max, st_opt;
	double d_d0;		/* normalized diffusivity in aggregated media */
	double ss;			/* solid phase of the aggregated medium, m3 m-3 bed volume */
	double a2;			/* intra-aggregate pore space, m3 m-3 bed volume */
...
void f_n2o_emit_casa(
	...
	double f_emit;		/* fraction of gas emission per mineralization */
	double n_total;
