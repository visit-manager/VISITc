...
void f_biolfix(
	...
	double aaa, f_tree, f_c3grass, f_c4grass;
	...
	/* tree-grass partitioning ratios */
	aaa = (mass->tree).rot + loct->funder_c3*(mass->c3).rot + loct->funder_c4*(mass->c4).rot;
	if(aaa > 0.0){
		f_tree = (mass->tree).rot / aaa;
		f_c3grass = loct->funder_c3*(mass->c3).rot / aaa;
		f_c4grass = loct->funder_c4*(mass->c4).rot / aaa;
	}

	(flux->tree).n_biofix = total_nbiofix * f_tree;
	(flux->c3).n_biofix = total_nbiofix * f_c3grass;
	(flux->c4).n_biofix = total_nbiofix * f_c4grass;
}
...
void f_n_leaching(
	...
	/* kg H2O / m2 */
	if(loct->soilwtr_l > 0.0){
		ntr_conc = 0.1 * (mass->n_no3/10000.0) / loct->soilwtr_l;
	}else{
		ntr_conc = 0.0;
	}
	/* g N / kg H2O */
...
void f_n_deposit(
	...
	/*	switch(grid->site_id){
		...
		if(loct->climy<=1850){
			...
			aa = loct->ndepo[0] + (loct->ndepo[1] - loct->ndepo[0])*(double)(loct->climy-1850)/143.0;
		
			ndepo_dry = 0.5 * aa /365.0;
			ndepo_wet = 0.5 * aa * (loct->prate_sfc + 1.0)/pre_ann;
		}else if(loct->climy>1993 && loct->climy<=2050){
			aa = loct->ndepo[1] + (loct->ndepo[2] - loct->ndepo[1])*(double)(loct->climy-1993)/57.0;
...
void f_n_uptake(
	...
	double f_temp;
	double n_max_no3 = 0.1;
	double n_max_nh4 = 0.1;
	double ks, navil;
	double uptake_no3, uptake_nh4;
	double aaa, f_tree, f_c3grass, f_c4grass;
	double bbb, fstor_tree, fstor_c3grass, fstor_c4grass;

	/* Reference
	Raich, J. W., et al. (1991), Potential net primary productivity in South America: 
	application of a global model, Ecological Applications, 1(4), 399-429.
	Appendix  Eq.1.16 */
	
	/*****
	Effect of N storage in plant body
	*****/
	/* tree */
	bbb = ((mass->tree).n_storage - (mass->tree).n_struct) / (echar->tree).n_max_mobile;
	if(bbb > 0.0 && bbb <=1.0){
		fstor_tree = 1.0 - bbb / (bbb + (echar->tree).n_max_mobile*0.4);
	}else{
		fstor_tree = 0.0;
	}
	/* c3 grass */
	bbb = ((mass->c3).n_storage - (mass->c3).n_struct) / (echar->c3).n_max_mobile;
	if(bbb > 0.0 && bbb <=1.0){
		fstor_c3grass = 1.0 - bbb / (bbb + (echar->c3).n_max_mobile*0.4);
	}else{
		fstor_c3grass = 0.0;
	}
	/* c4 grass */
	bbb = ((mass->c4).n_storage - (mass->c4).n_struct) / (echar->c4).n_max_mobile;
	if(bbb > 0.0 && bbb <=1.0){
		fstor_c4grass = 1.0 - bbb / (bbb + (echar->c4).n_max_mobile*0.4);
	}else{
		fstor_c4grass = 0.0;
	}
	...
	uptake_no3 = navil * n_max_no3 * ks / (1.0 + ks*navil) * f_temp;
	/* g N ha-1 day-1 */
	(flux->tree).uptake_no3 = f_tree * fstor_tree * uptake_no3 * 10000.0;
	(flux->c3).uptake_no3 = f_c3grass * fstor_c3grass * uptake_no3 * 10000.0;
	(flux->c4).uptake_no3 = f_c4grass * fstor_c4grass * uptake_no3 * 10000.0;
	
	/* NH4 uptake */
	navil = (mass->soil).n_nh4/10000.0;
	uptake_nh4 = navil * n_max_nh4 * ks / (1.0 + ks*navil) * f_temp;
	/* g N ha-1 day-1 */
	(flux->tree).uptake_nh4 = f_tree * fstor_tree * uptake_nh4 * 10000.0;
	(flux->c3).uptake_nh4 = f_c3grass * fstor_c3grass * uptake_nh4 * 10000.0;
	(flux->c4).uptake_nh4 = f_c4grass * fstor_c4grass * uptake_nh4 * 10000.0;
}
...
void f_n_abandon_salvage(
	...
	double aaa, bbb, cn_storage;

	pchar->n_salvage = 0.3;
	pchar->cn0_strg = 100.0;
	
	cn_storage = mass->n_storage / (mass->stm + mass->rot) / 1000000.0;

	/* canopy *******/
	/* abandon */
	flux->n_abdn_cnpy = (1.0 - pchar->n_salvage) * pchar->lf * mass->n_canopy;
	/* salvage (draw back to storage) */
	flux->n_salvage = pchar->n_salvage * pchar->lf * mass->n_canopy;

	/* storage *******/
	if((mass->stm + mass->rot) > 0.0){
		aaa = mass->stm / (mass->stm + mass->rot);
		bbb = mass->rot / (mass->stm + mass->rot);
	}else{
		aaa = bbb = 0.0;
	}
	
	flux->n_abdn_strg = (aaa*pchar->lc + bbb*pchar->lr)*mass->n_storage;
	
	if(flux->n_abdn_strg > mass->n_storage*0.02){
		flux->n_abdn_strg = mass->n_storage*0.02;
	}
}
...
void f_n_immoblz(
	...
	/* flux->n_immbl = 0.10 * flux->n_minerlz_lttr + 0.6 * flux->n_minerlz_hums +
		(0.004 * mass->n_no3 + 0.04 * mass->n_nh4);*/ /* TMK */

	flux->n_immbl = 0.01 * flux->n_minerlz_lttr + 0.1 * flux->n_minerlz_hums +
		(0.004 * mass->n_no3 + 0.04 * mass->n_nh4); /* PSO */
...
void f_n_mcrb_abdn(
	...
	flux->n_mcrb_abdn = 0.80 * f_temp * mass->n_mcrb;  /*  TKY */
...
void f_n_alloc(
	...
	kn = pchar->kn_nphoto = 70.0;
	rd_nsp = pchar->rd_n = amax*0.003;
...
void f_n_mineralz(
	...
	/* g N ha-1 month-1 */
	flux->n_minerlz_lttr = mass->n_lttr * f_c_min * 0.1;  /*  */
	...
	/* g N ha-1 day-1 */
	flux->n_minerlz_hums = mass->n_hums * f_c_min * 0.1;
}
