...
void f_ch4emit_plant(
	...
	sunshine = loct->daylen[loct->doy] * (1.0 - loct->tcdc_clm);
	if(sunshine<0.0){
		sunshine = 0.0;
	}
...
void f_ch4emit_walter(
	...
	double nppmax, f_in, f_org[SOIL_LAYER+2], f_t[SOIL_LAYER+2], f_grow, t_gr, t_mat;
	double t_veg, flux_ebull, flux_plant;
	double hh, rr, kk, df_dry, release;
    ...
    /* rooting depth (assumption), m */
	/* rdepth = 0.3; */
    rdepth = loct->rooting_depth;
    
    /* porosity */
	poro = 0.45*grid->sand_frac + 0.20*(1.0 - grid->sand_frac - grid->clay_frac)
			+0.14*grid->clay_frac + 0.45*0.03;		
	...
	t_mat = t_gr + 10.0;
	for(f=1;f<=SOIL_LAYER;f++){
		tmp[f] = loct->tmp10_soil*(double)(SOIL_LAYER - f)/(double)SOIL_LAYER 
					+ (double)f/(double)SOIL_LAYER*loct->tmp200_soil;
	}
	/* fgrow: Eq. 20 */
	if(tmp[5] < t_gr){
		f_grow = 0.0;
	...
	for(g=1;g<=cumtime;g++){ 
		...
		ww[SOIL_LAYER] = loct->prof_ch4[SOIL_LAYER] + rr*(df[SOIL_LAYER]*(2.0*loct->prof_ch4[SOIL_LAYER-1] - 2.0*loct->prof_ch4[SOIL_LAYER]));
	...
	(flux->soil).ch4_wh_diff =  df[1]/(dpth[1]-dpth[0])*(loct->prof_ch4[1]-loct->prof_ch4[0]) *24.0*16.0/1000.0 ; /* + release *24.0 * 16.0 / 1000.0 */
}
