...
int main(int argc, char *argv[])
{
	...
	static char title_text[] = "VISIT+CFSR map"; 
    ...
    fp_in = fopen("CFSR_V01_1995_140425.flt","rb");
    for(f=0;f<24*365;f++){
        f_doyTmody(1979,f/24, &month,&day);
    
        fread(mread,sizeof(float),NLAT*NLON, fp_in);
        
        for(g=0;g<NLAT*NLON;g++){
            mans[g] += mread[g];
            mans[(month+1)*NLAT*NLON + g] += mread[g];
        }
    }
    fclose(fp_in);
    ...
    strcpy(filename, "VISITc_V01_1995_140425_mon.nc");
