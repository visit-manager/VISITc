...
void f_cycle_soil(
	...
	float degrade_tf, degrade_tc, degrade_tr;
	float degrade_gf, degrade_gc, degrade_gr;
	float degrade_ha, degrade_hi, degrade_hp;
	float hf_a, hf_i, hf_p;
	float d13c_hf_a,  d13c_hf_i,  d13c_hf_p;
	float d13c_mrl, d13c_mrh;
	float d13c_ltr, d13c_msl;
	float aaa;
	
	aaa = mass->msl_a;
	
	if(aaa>=0.0 && aaa<=1000.0){
		;
	}else{
		printf("!!!---WRONG    ACTIVE  HUMUS---!!! 0: %f %f %f %d\n", aaa, mass->msl_a, schar->f_tm_h, grid->veg_type);
		//printf("!!!---WRONG    ACTIVE  HUMUS---!!!: %f %f %f %d\n", mass->msl_a, hf_a, degrade_ha, grid->veg_type);
		//mass->msl_a = 0.0;
	}
	...
	degrade_tf = mass->ltr_tf * schar->sr_lf/1000.0 / (double)DSTEP * schar->f_tm_l;
	degrade_tc = mass->ltr_tc * schar->sr_lc/1000.0 / (double)DSTEP * schar->f_tm_l;
	degrade_tr = mass->ltr_tr * schar->sr_lr/1000.0 / (double)DSTEP * schar->f_tm_l;	
	degrade_gf = mass->ltr_gf * schar->sr_lf/1000.0 / (double)DSTEP * schar->f_tm_l;
	degrade_gc = mass->ltr_gc * schar->sr_lc/1000.0 / (double)DSTEP * schar->f_tm_l;
	degrade_gr = mass->ltr_gr * schar->sr_lr/1000.0 / (double)DSTEP * schar->f_tm_l;	
	degrade_ha = mass->msl_a * schar->sr_ha/1000.0 / (double)DSTEP * schar->f_tm_h;
	degrade_hi = mass->msl_i * schar->sr_hi/1000.0 / (double)DSTEP * schar->f_tm_h;
	degrade_hp = mass->msl_p * schar->sr_hp/1000.0 / (double)DSTEP * schar->f_tm_h;
	...
	if(mass->ltr_tf<0.0){
		//printf("!!!---ZERO    TREE  LEAF  LITTER---!!!: %f\n", mass->ltr_tf);
		mass->ltr_tf = 0.0;
	}
	if(mass->ltr_tc<0.0){
		//printf("!!!---NEGATIVE    TREE  STEM  LITTER---!!!: %f\n", mass->ltr_tc);
		mass->ltr_tc = 0.0;
	}
	if(mass->ltr_tr<0.0){
		//printf("!!!---NEGATIVE    TREE  ROOT  LITTER---!!!: %f\n", mass->ltr_tr);
		mass->ltr_tr = 0.0;
	}
	if(mass->ltr_gf<0.0){
		//printf("!!!---NEGATIVE    GRASS  LEAF  LITTER---!!!: %f\n", mass->ltr_gf);
		mass->ltr_gf = 0.0;
	}
	if(mass->ltr_gc<0.0){
		//printf("!!!---NEGATIVE     GRASS  STEM  LITTER---!!!: %f\n", mass->ltr_gc);
		mass->ltr_gc = 0.0;
	}
	if(mass->ltr_gr<0.0){
		//printf("!!!---NEGATIVE     GRASS  ROOT  LITTER---!!!: %f\n", mass->ltr_gr);
		mass->ltr_gr = 0.0;
	}
	...
	if(aaa>=0.0 && aaa<=1000.0){
		...
		//printf("!!!---WRONG    ACTIVE  HUMUS---!!! 1: %f %f %f %d\n", aaa, mass->msl_a, schar->f_tm_h, grid->veg_type);
		//printf("!!!---WRONG    ACTIVE  HUMUS---!!!: %f %f %f %d\n", mass->msl_a, hf_a, degrade_ha, grid->veg_type);
		//mass->msl_a = 0.0;
	}
	...
	if(mass->msl_i<0.0){
		//printf("!!!---NEGATIVE    INT.  HUMUS---!!!: %f\n", mass->msl_i);
		mass->msl_i = 0.0;
	}
	if(mass->msl_p<0.0){
		//printf("!!!---NEGATIVE    PASSIVE  HUMUS---!!!: %f\n", mass->msl_p);
		mass->msl_p = 0.0;
	}  
