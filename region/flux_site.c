...
void f_flux_TKY(
	...
	long h;
	long yr, day,need_flag, needt_flag;
	float hr, assim_h, ecoresp_h, apar_h, scale;
	float vp, vps, vpd;
	float fgppd, ferd, fneed, fneed_obs;
	float fgppdt, ferdt, fneedt, fneedt_obs, fapardt;
	float obs_neeh, obs_uw, ustar;
	float obs_atmp;
	float obs_stmp[6], obs_swtr[4];
	float obs_par_in;
	float obs_rhmd;
	float fatmp, fppfd;
	
	float apar_bn, apar_qn, apar_un;
	float apar_bd, apar_qd, apar_ud;
	float gpp_bn, gpp_qn, gpp_un;
	float gpp_bd, gpp_qd, gpp_ud;
	float pc_bn, pc_qn, pc_un;
	float pc_bd, pc_qd, pc_ud;
	float pj_bn, pj_qn, pj_un;
	float pj_bd, pj_qd, pj_ud;
	float rd_bn, rd_qn, rd_un;
	float rd_bd, rd_qd, rd_ud;
	
	float ipar_d, apar_d, gpp_d, tmp_d, er_d;
	float vcmax_av, jmax_av, Iabs_av, j_av, ci_av, gs_av, ac_av, aj_av, rd_av;
	float limit_ac[4], limit_aj[4];
	
	scale = 1800.0/1000000.0;
	/* 1800.0 seconds: 30 minute */
	/* 1000000.0 micro mol: mol */
	
	if(loct->climy>=1998 && loct->climy<=2005){
		...
		for(h=0;h<DSTEP;h++){
			loct->hour = h;
			loct->hangle = -180.0+(float)h*7.5;
			loct->metdat_frag = 0;
			
			/*** environmental condition ************************************/				
			fscanf(fp_o,"%ld %ld %f", &yr, &day, &hr);
			fscanf(fp_o,"%f %f", &obs_atmp, &obs_rhmd);
			fscanf(fp_o,"%f %f %f %f %f", &obs_stmp[0], &obs_stmp[1], 
					&obs_stmp[2], &obs_stmp[3], &obs_stmp[4]);
			fscanf(fp_o,"%f %f %f %f", &obs_swtr[0], &obs_swtr[1], 
					&obs_swtr[2], &obs_swtr[3]);
			fscanf(fp_o,"%f %f %f", &obs_par_in, &obs_uw, &obs_neeh);
			...
			if(obs_par_in>-50.0&&obs_par_in<3000.0){
				...
				if(loct->ppfd_h>=0.0 && obs_par_in>=0.0){
					(echar->overs1).ppfdb_top = (echar->overs2).ppfdb_top = obs_par_in 
												* loct->ppfdb_h/loct->ppfd_c_h;
					(echar->overs1).ppfdd_top = (echar->overs2).ppfdd_top = obs_par_in 
												* loct->ppfdd_h/loct->ppfd_c_h;
				}else{
				...
				if(obs_par_in>-50.0&&obs_par_in<3000.0){
					...
					(echar->overs1).ppfdb_top = (echar->overs2).ppfdb_top = loct->ppfdb_h;
					(echar->overs1).ppfdd_top = (echar->overs2).ppfdd_top = loct->ppfdd_h;
					fppfd = loct->ppfdb_h + loct->ppfdd_h;
				}
			}
			...
			f_gpp_sunshade((mass->tree).season, grid, loct, &(echar->overs1));
			f_gpp_sunshade((mass->tree).season, grid, loct, &(echar->overs2));
			if(mass->snwa<20.0){
				/* snow-free */
				f_gpp_sunshade((mass->c3).season, grid, loct, &(echar->unders));
				f_gpp_sunshade((mass->c3).season, grid, loct, &(echar->unders));
			}else{
				/* snow-covered */
			...
			gpp_d += assim_h*scale;
			tmp_d += (echar->overs1).tmp/(float)DSTEP;
			...
			/*  daytime  */
			if(fppfd>50.0&&loct->metdat_frag==7&&ustar>0.3&&obs_neeh>-100.0&&obs_neeh<100.0){ /*  g C m-2 day-1  */
				needt_flag++;  /*  &&ustar>0.3   &&obs_neeh>-100.0&&obs_neeh<100.0 */
		...
		/*	if(ippfd>50.0){  */
			...
			/**  hourly property  *****************************/
		/*	if(loct->month==7 && loct->mday<10){
				fprintf(fp_r[loct->climy-1998+9],"%ld %ld %ld %ld %ld %ld ", 
					loct->climy, loct->doy, loct->month+1, loct->mday+1, h, loct->metdat_frag);  
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f %f %f ",(echar->overs1).tmp, 
					(echar->overs1).ppfdb_top, (echar->overs1).ppfdd_top, (echar->overs1).vpd_a);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).lai_sn,(echar->overs1).lai_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).lai_sn,(echar->overs2).lai_sd);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).vcmx_sn,(echar->overs1).vcmx_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).vcmx_sn,(echar->overs2).vcmx_sd);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).jmx_sn,(echar->overs1).jmx_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).jmx_sn,(echar->overs2).jmx_sd);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).ci_sn,(echar->overs1).ci_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).ci_sn,(echar->overs2).ci_sd);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).gs_sn,(echar->overs1).gs_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).gs_sn,(echar->overs2).gs_sd);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).j_sn,(echar->overs1).j_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).j_sn,(echar->overs2).j_sd);
				
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).pc_sn,(echar->overs1).pc_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).pc_sn,(echar->overs2).pc_sd);

				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).pj_sn,(echar->overs1).pj_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).pj_sn,(echar->overs2).pj_sd);

				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).p_sn,(echar->overs1).p_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).p_sn,(echar->overs2).p_sd);

				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).rd_sn,(echar->overs1).rd_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).rd_sn,(echar->overs2).rd_sd);

				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs1).a_sn,(echar->overs1).a_sd);
				fprintf(fp_r[loct->climy-1998+9],"%f %f ",(echar->overs2).a_sn,(echar->overs2).a_sd);
		...
		/*	fprintf(fp_r[grid->year-1998+3],"%f %f %f ",(echar->overs1).a_sd,(echar->overs1).p_sd, (echar->overs1).rd_sd);
			fprintf(fp_r[grid->year-1998+3],"%f %f ",(echar->overs1).pc_sd,(echar->overs1).pj_sd);
			fprintf(fp_r[grid->year-1998+3],"%f %f ",(echar->overs1).gs_sd,(echar->overs1).gb);
			fprintf(fp_r[grid->year-1998+3],"%f %f ",(echar->overs1).co2_a,(echar->overs1).ci_sd);
			
			fprintf(fp_r[grid->year-1998+3],"%f %f ",(echar->overs1).vcmx_sd,(echar->overs1).comp_co2);
			fprintf(fp_r[grid->year-1998+3],"%f %f ",(echar->overs1).k_effc,(echar->overs1).j_sd);   */
			
		/*	fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->overs1).a_sn,(echar->overs1).p_sn, (echar->overs1).rd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).pc_sn,(echar->overs1).pj_sn);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).gs_sn,(echar->overs1).gb);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).co2_a,(echar->overs1).ci_sn);  */
			
		/*	fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).lai_sn, (echar->overs1).lai_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs2).lai_sn, (echar->overs2).lai_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->unders).lai_sn, (echar->unders).lai_sd);

			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).apar_sn, (echar->overs1).apar_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs2).apar_sn, (echar->overs2).apar_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->unders).apar_sn, (echar->unders).apar_sd);
			
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).j_sn,(echar->overs1).j_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs2).j_sn,(echar->overs2).j_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->unders).j_sn,(echar->unders).j_sd);
			
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->overs1).pc_sn,(echar->overs1).pj_sn, (echar->overs1).p_sn);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->overs1).pc_sd,(echar->overs1).pj_sd, (echar->overs1).p_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->overs2).pc_sn,(echar->overs2).pj_sn, (echar->overs2).p_sn);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->overs2).pc_sd,(echar->overs2).pj_sd, (echar->overs2).p_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->unders).pc_sn,(echar->unders).pj_sn, (echar->unders).p_sn);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f %.2f ",(echar->unders).pc_sd,(echar->unders).pj_sd, (echar->unders).p_sd);

			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).gs_sn,(echar->overs1).gs_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs2).gs_sn,(echar->overs2).gs_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->unders).gs_sn,(echar->unders).gs_sd);
			
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs1).ci_sn,(echar->overs1).ci_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->overs2).ci_sn,(echar->overs2).ci_sd);
			fprintf(fp_r[grid->year-1998+3],"%.2f %.2f ",(echar->unders).ci_sn,(echar->unders).ci_sd);
			
			fprintf(fp_r[grid->year-1998+3],"%.2f ",(echar->overs1).rd);
			fprintf(fp_r[grid->year-1998+3],"%.2f ",(echar->overs2).rd);
			fprintf(fp_r[grid->year-1998+3],"%.2f ",(echar->unders).rd);  */
			...
			fprintf(fp_r[3],"%12.4f ", ustar);
			fprintf(fp_r[3],"%12.4f ", apar_h*scale);
			fprintf(fp_r[3],"%12.4f ",loct->ppfdb_h+loct->ppfdd_h);
			fprintf(fp_r[3],"%12.4f ", assim_h*scale*12.0);
			fprintf(fp_r[3],"%12.4f ", ecoresp_h*scale*12.0);
			fprintf(fp_r[3],"%12.4f ", ecoresp_h*scale*12.0 - assim_h*scale*12.0);
			fprintf(fp_r[3],"%12.4f ", obs_neeh*scale*12.0);
			fprintf(fp_r[3],"%12.4f ", apar_h*scale);
		...
		fprintf(fp_r[4],"%4ld ", (mass->tree).season);
		fprintf(fp_r[4],"%f ", ipar_d);
		fprintf(fp_r[4],"%f ", apar_d);
	/* 	fprintf(fp_r[4],"%12.6f ", (echar->overs1).vcmx_b*(echar->overs1).vcmx_con); 
		fprintf(fp_r[4],"%12.6f ", (echar->overs1).jvr); 
		fprintf(fp_r[4],"%12.6f ", (echar->overs1).lma); 
		fprintf(fp_r[4],"%12.6f ", (echar->overs1).rd0);    */
	/*	fprintf(fp_r[4],"%12.6f ", (echar->overs2).vcmx_b*(echar->overs1).vcmx_con); 
		fprintf(fp_r[4],"%12.6f ", (echar->overs2).jvr); 
		fprintf(fp_r[4],"%12.6f ", (echar->overs2).lma); 
		fprintf(fp_r[4],"%12.6f ", (echar->overs2).rd0);     */
		/*  continuous 1 */
		fprintf(fp_r[4],"%12.6f %12.6f %12.6f ", fgppd, ferd, fneed);   /*  */
		/*  continuous 2 */
		if(need_flag>=45){  /*  need_flag==48  */
			...
			fprintf(fp_r[4],"%12.6f %12.6f %12.6f ", 
		...
		/*  fprintf(fp_r[4],"%12.6f %12.6f %12.6f ", (echar->overs1).lai, (echar->overs2).lai, (echar->unders).lai);   */ 
		/*  only good day  */
		if(need_flag>=45){  /*  need_flag==48  */
			fprintf(fp_r[4],"%12.6f %12.6f %12.6f %12.6f ", fgppd, ferd, fneed, fneed_obs);   /*  */ 
		}else{
			;
		}
		fprintf(fp_r[4],"\n");   /*  */
		...
		if(needt_flag>=1){
			...
			fprintf(fp_r[5],"%f ", fapardt);
			fprintf(fp_r[5],"%f %f ", fgppdt, ferdt);
			fprintf(fp_r[5],"%f %f ", fneedt, fneedt_obs);
			fprintf(fp_r[5],"\n");   /*  */
		}else{
			fprintf(fp_r[5],"%4ld %4ld %4ld %4ld ", loct->climy, loct->month, loct->doy, needt_flag);
			fprintf(fp_r[5],"\n");   /*  */
		...
		/**  daily canopy  *****************************/
		fprintf(fp_r[6],"%4ld %4ld %4ld %4ld ", loct->climy, loct->doy, (mass->tree).day_frush, needt_flag);  
		fprintf(fp_r[6],"%f %f %f ", apar_bn, apar_qn, apar_un);
		fprintf(fp_r[6],"%f %f %f ", apar_bd, apar_qd, apar_ud);
		fprintf(fp_r[6],"%f %f %f ", gpp_bn, gpp_qn, gpp_un);
		fprintf(fp_r[6],"%f %f %f ", gpp_bd, gpp_qd, gpp_ud);
		fprintf(fp_r[6],"%f %f %f ", pc_bn, pc_qn, pc_un);
		fprintf(fp_r[6],"%f %f %f ", pc_bd, pc_qd, pc_ud);
		fprintf(fp_r[6],"%f %f %f ", pj_bn, pj_qn, pj_un);
		fprintf(fp_r[6],"%f %f %f ", pj_bd, pj_qd, pj_ud);
		fprintf(fp_r[6],"%f %f %f ", rd_bn, rd_qn, rd_un);
		fprintf(fp_r[6],"%f %f %f ", rd_bd, rd_qd, rd_ud);
		
		fprintf(fp_r[6],"%f %f %f %f %f ", ipar_d, apar_d, gpp_d, tmp_d, er_d);
		fprintf(fp_r[6],"%f %f %f %f %f %f %f ", vcmax_av, jmax_av, Iabs_av, j_av, ac_av, aj_av, rd_av); /* */

	/*	fprintf(fp_r[6],"%f %f ", (gpp_bn+gpp_bd)*12.0, (gpp_qn+gpp_qd)*12.0);
		fprintf(fp_r[6],"%f %f ", (gpp_bn+gpp_bd-rd_bn-rd_bd)*12.0, (gpp_qn+gpp_qd-rd_qn-rd_qd)*12.0);  */
		
	/*	fprintf(fp_r[6],"%f %f %f %f %f %f %f %f ", 
